...
# change consecutive ranges of used UIDs to ranges
uids.sort()
uids_ranges = ",".join(
    as_range(g) for _, g in groupby(uids, key=lambda n, c=count(): n - next(c))
)

gids.sort()
gids_ranges = ",".join(
    as_range(g) for _, g in groupby(gids, key=lambda n, c=count(): n - next(c))
)
print("used UIDs ranges", uids_ranges)
print("used GIDs ranges", gids_ranges)
